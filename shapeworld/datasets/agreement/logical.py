...
from shapeworld.captioners import CaptionerMixer, RegularAttributeCaptioner, RegularTypeCaptioner, AttributeTypeRelationCaptioner, RelationCaptioner, ExistentialCaptioner, QuantifierCaptioner, ConjunctionCaptioner, DisjunctionCaptioner
...
class LogicalDataset(CaptionAgreementDataset):
    ...
    def __init__(
        ...
        if captions is None or 'existential' in captions:
            ...
            captioners.append(existential_captioner)

        if captions is None or 'relational' in captions:
            ...
            captioners.append(relational_captioner)

        if captions is None or 'quantification' in captions:
            quantification_captioner = QuantifierCaptioner(
                restrictor_captioner=restrictor_captioner,
                body_captioner=body_captioner
            )
            captioners.append(quantification_captioner)

        captioner = CaptionerMixer(
            captioners=captioners,
            distribution=[1 for _ in captioners]
        )

        world_captioner = CaptionerMixer(
            captioners=(
                ConjunctionCaptioner(captioner=captioner),
                DisjunctionCaptioner(captioner=captioner)
            )
        )
